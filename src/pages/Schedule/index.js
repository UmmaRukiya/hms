...
function Schedule() {
    ...
    const deleteData = (id) => {
        axios.delete(`${process.env.REACT_APP_API_URL}/schedule/${id}`).then(function(response){
            getDatas();
        });
    }
  return (
                        ...
                        <div className="card-content">
                            <div className="table-responsive">
                                <table className="table table-bordered mb-0">
                                    ...
                                    {data && data.map((d, key) =>
                                        <tr key={d.id}>
                                            <td>{d.emplye?.name_en}</td>
                                            <td>{d.day?.day_name}</td>
                                            <td>{d.shift?.shift_name}</td>
                                            <td>{d.status}</td>
                                            <td>
                                                <Link to={`/schedule/edit/${d.id}`} className='btn btn-info' ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-pencil-square" viewBox="0 0 16 16">
