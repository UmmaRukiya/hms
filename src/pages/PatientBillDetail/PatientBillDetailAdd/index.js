...
function RoomListAdd() {
    const [inputs, setInputs] = useState({id:'', patient_bill_id:'', particular:'', amount:''});
    const [patientbill, setPatientbill] = useState([]);
    const navigate=useNavigate();
    const {id} = useParams();
    
    function getDatas(){
        axios.get(`${process.env.REACT_APP_API_URL}/patientbilldetail/${id}`).then(function(response) {
            setInputs(response.data.data);
        });
    }
    function get_relation(){
        axios.get(`${process.env.REACT_APP_API_URL}/patientbill/index`).then(function(response) {
            setPatientbill(response.data.data);
        });
    ...
    const handleSubmit = async(e) => {
        ...
        try{
            let apiurl='';
            if(inputs.id!=''){
                apiurl=`/patientbilldetail/edit/${inputs.id}`;
            }else{
                apiurl=`/patientbilldetail/create`;
            }
                                        ...
                                        <div className="form-body">
                                            <div className="row">
                                                <div className="col-12">
                                                    ...
                                                    <label htmlFor="first-name-vertical">Patient Bill ID</label>
                                                    {patientbill.length > 0 && 
