...
function PatientTest() {
    ...
    function getDatas() {
        axios.get(`${process.env.REACT_APP_API_URL}/patienttest/index`).then(function(response) {
            setData(response.data.data);
        });
    }
    const deleteData = (id) => {
        axios.delete(`${process.env.REACT_APP_API_URL}/patienttest/${id}`).then(function(response){
            getDatas();
        });
    }
    const generatePrintableTest = (test) => {
        const printContent = `
            <html>
            <head>
                <meta charset="UTF-8">
            ...
            <body>
                <div class="container">
                    ...
                    <div class="row border p-2">
                        <div class="col-8">
                            <div>Patient Name: ${test.patient?.name}</div>
                            <div>Age: ${test.age}</div>
                            <div>Gender: ${test.patient?.gender || 'N/A'}</div>
                            <div>Address: ${test.patient?.present_address || 'N/A'}</div>
                            <div>Father/Husband: ${test.patient?.father_name || 'N/A'}</div>
                        </div>
                        <div class="col-4">
                            ...
                            <div>Bill Date: ${new Date(test.bill_date).toLocaleDateString()}</div>
                        </div>
                    </div>
                    ...
                    ${test.detail && test.detail.length > 0 
                        ? test.detail.map(detail => `
                            <div class="row border p-2">
                    ...
                    <div class="row d-flex justify-content-end">
                        ...
                        <div class="col-3">${test.discount || 'N/A'}</div>
                    </div>
    
                    <div class="row d-flex justify-content-end">
                        <div class="col-3 fw-bold">Tax (%):</div>
                        <div class="col-3">${test.vat || 'N/A'}</div>
                    </div>
    
                    <div class="row d-flex justify-content-end">
                        <div class="col-3 fw-bold">Total:</div>
                        <div class="col-3">${test.total_amount || 'N/A'}</div>
                    </div>
                    <div class="row d-flex justify-content-end">
                        <div class="col-3 fw-bold">Paid:</div>
                        <div class="col-3">${test.paid || 'N/A'}</div>
                    </div>
                </div>
            </body>
            </html>
                ...
                <div className="col-12">
                    <div className="card">
                        <div className="card-header">
                            <h4 className="card-title">All Patient Test</h4>
                            <Link to={'/patienttest/add'} className='btn btn-primary float-right' >Add New</Link>
                        </div>
                                ...
                                <table className="table table-bordered mb-0">
                                    ...
                                    {data && data.map((d, key) =>
                                        <tr key={d.id}>
                                            ...
                                            <td>{d.paid}</td>
                                            <td>
                                            <button type='button' onClick={() => generatePrintableTest(d)} className='btn btn-outline-secondary'> 
